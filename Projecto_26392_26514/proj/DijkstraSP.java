...
/*import edu.ufp.lp2.proj.EdgeWeightedDigraph;*/
/**
...
public class DijkstraSP {
    ...
    //RedBlackBST_Projecto<Integer, Aviao> avioesST = new RedBlackBST_Projecto<>();
    //RedBlackBST_Projecto<edu.ufp.lp2.proj.Date, Voo> vooST = new RedBlackBST_Projecto<>();
    //SeparateChainingHashST_Projecto<String, Aeroporto> aeroportoST = new SeparateChainingHashST_Projecto<>();
    /**
     * Computes a shortest-paths tree from the source vertex {@code s} to every
     * other vertex in the edge-weighted digraph {@code G}. Função alterada para
     * calcular o caminho mais curto, o caminho mais economico e o mais curto
     ...
     */
    public DijkstraSP(EdgeWeightedDigraph G, int s, Aviao aviao, String op) {
    ...
    private void relax(DirectedEdge e, Aviao aviao, String op) {
        int v = e.from(), w = e.to();
        double custo = 0;
        switch (op) {
            case "Duracao":
                custo = aviao.calcTempo(e);
                break;
            case "Consumo":
                custo = aviao.calcConsumo(e);
                break;
            default:
                custo = e.weight();
        }
        if (distTo[w] > distTo[v] + custo) {
            distTo[w] = distTo[v] + custo;
            edgeTo[w] = e;
            if (pq.contains(w)) {
